...
for i in range(len(mean_dict["Ts_hcor"])):
    if mean_dict["Ts_hcor"][i] > indoor_temp:
        ...
        if deltaT > 0:
            tmp[i] = np.exp(-0.5*((data_dict["Ts_hcor"][i]-indoor_temp)/sigma)*((data_dict["Ts_hcor"][i]-indoor_temp)/sigma))
        elif deltaT < 0:
            tmp[i] = 0
        else:
            print("wat")
...
sigma = 1#np.nanstd(data_dict["q"]-indoor_q)
for i in range(len(mean_dict["q"])):
    if mean_dict["q"][i] > indoor_q:
        
        deltaq = data_dict["q"][i] - indoor_q
        if deltaq > 0:
            tmp2[i] = np.exp(-0.5*((data_dict["q"][i]-indoor_q)/sigma)*((data_dict["q"][i]-indoor_q)/sigma))
        elif deltaq < 0:
            tmp2[i] = 0
        else:
            print("wat")
...
index = np.where(sum_arr > 1.4)

for var in variables:
    
    data_dict[var][index] = mean_dict[var][index]*1
...
for i in range(seperation-1):
    for j,var in enumerate(variables):
        ...
        #ax2 = ax[j].twinx()
        #if var == "Ts_hcor":
            #ax2.plot(interval,w_dict[var][dt[i]:dt[i+1]]+tmp[dt[i]:dt[i+1]],linewidth=0.1,color = "b")
        #elif var == "q":
            #ax2.plot(interval,w_dict[var][dt[i]:dt[i+1]]+tmp2[dt[i]:dt[i+1]],linewidth=0.1,color = "b")
        #else:
